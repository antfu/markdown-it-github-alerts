...
const markerRe = /^\[\!(TIP|NOTE|IMPORTANT|WARNING|CAUTION)\]\s/
...
const MarkdownItGitHubAlerts: MarkdownIt.PluginWithOptions<MarkdownItGitHubAlertsOptions> = (md, _options = {}) => {
  md.core.ruler.after('block', 'github-alerts', (state) => {
    ...
    for (let i = 0; i < tokens.length; i++) {
      if (tokens[i].type === 'blockquote_open') {
        ...
        firstContent.content = firstContent.content.replace(markerRe, '').trimStart()
        open.type = 'html_block'
        open.tag = 'div'
        open.content = `<div class="markdown-alert markdown-alert-${type}"><p class="markdown-alert-title" dir="auto">${icons[type]}${capitalize(type)}</p>`
